...
static const phy_media_member_info_t  media_mem_info[] = {
    ...
    {BASE_PAS_MEDIA_VENDOR_ID, offsetof(pas_media_t, vendor_id),
        MEMEBER_SIZEOF(pas_media_t, vendor_id), cps_api_object_ATTR_T_BIN},
    ...
    {BASE_PAS_MEDIA_VENDOR_REV, offsetof(pas_media_t, vendor_rev),
        MEMEBER_SIZEOF(pas_media_t, vendor_rev), cps_api_object_ATTR_T_BIN},
    ...
    {BASE_PAS_MEDIA_DATE_CODE, offsetof(pas_media_t, date_code),
        MEMEBER_SIZEOF(pas_media_t, date_code), cps_api_object_ATTR_T_BIN},
...
{
    if ((phy_media_tbl == NULL) || (port < PAS_MEDIA_START_PORT) 
            || (port > phy_media_count)){

        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid port or media not \
                initialized,phy_media_tbl(%p) port (%d) phy_media_count(%d)\n",
                phy_media_tbl, port, phy_media_count);

        return NULL;
    }

    if ((phy_media_tbl[port].res_hdl == NULL)
            || (phy_media_tbl[port].res_data == NULL)) {

        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Not initialized, port(%d)\
                res_hdl(%p), res_data(%p)\n", port, phy_media_tbl[port].res_hdl,
                phy_media_tbl[port].res_data);
...
static bool dn_media_obj_publish (cps_api_object_t obj)
{
    ...
    if (obj == CPS_API_OBJECT_NULL) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid param, obj(%p)\n", obj);
...
bool dn_pas_phy_media_init (void) 
{
    ...
    if ((ent_count == 0) || (phy_media_count == 0)) {
        EV_LOG_ERR(ev_log_t_PAS, 3, "PAS", "Not able to get entity count \
                and resource count, Entity count(%u), res count(%u)\n",
                ent_count, res_count);
        return false;
    }
    ...
    if (phy_media_tbl == NULL) {
        ...
        EV_LOG_ERR(ev_log_t_PAS, 3, "PAS", "%s - %d: Memory allocation failed,\
                phy_media_tbl(%p)\n", __FUNCTION__, __LINE__, phy_media_tbl);
    ...
    if (res_count != phy_media_count) {
        ...
        EV_LOG_ERR(ev_log_t_PAS, 3, "PAS", 
                "%s - %d: Not able to get all the handles, (%d - %d)\n",
                __FUNCTION__, __LINE__, phy_media_count, res_count);
...
static bool dn_media_data_store_init (uint_t count)
{
    ...
    for (cnt = PAS_MEDIA_START_PORT; cnt <= count; cnt++) {
        ...
        cps_api_key_init(&key, cps_api_qualifier_OBSERVED,
                cps_api_obj_CAT_BASE_PAS, BASE_PAS_MEDIA_OBJ,
                PAS_MEDIA_INST_KEY_LEN, slot, cnt);

        if (dn_pas_res_insert(&key, phy_media_tbl[cnt].res_data) == false) {

            EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Data store insert failed, \
                    port(%d)\n", cnt);
    ...
    do {
        ...
        if (dn_pas_phy_media_is_present(port) == true) {
            if (dn_pas_media_obj_all_attr_add(media_mem_info,
                        ...
                        mtbl->res_data) == false) {
                EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Adding attribute \
                        list failed\n");
                break;
            }
        } else {
        ...
        PLATFORM_MEDIA_CATEGORY_t pcategory)
{
    ...
    if (count) {
        ...
        for (channel = PAS_MEDIA_CH_START; channel < count; channel++) {
            ...
            cps_api_key_init(&key, cps_api_qualifier_OBSERVED,
                    cps_api_obj_CAT_BASE_PAS, BASE_PAS_MEDIA_CHANNEL_OBJ,
                    PAS_MEDIA_CHANNEL_INST_KEY_LEN, slot, port, channel);

            if (dn_pas_res_insert(&key, &ch_data[channel]) == false) {
                 EV_LOG_ERR(ev_log_t_PAS, 3, "PAS", "%s - %d: Data store insert \
                         failed for channel(%u) data.\n", __FUNCTION__,
                         __LINE__, channel);
                ret = false;
            }
        }
...
static bool dn_pas_media_presence_poll (uint_t port, cps_api_object_t obj)
{
    ...
    if (pas_sdi_media_presence_get(mtbl->res_hdl, &presence) 
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid SDI handle, \
                port(%d) handle (%p)\n", port, mtbl->res_hdl);
    ...
    if (mtbl->res_data->present != presence) {
        ...
        if (obj != NULL) {
            ...
            if (cps_api_object_attr_add(obj, BASE_PAS_MEDIA_PRESENT,
                ...
                EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Function \
                        cps_api_object_attr_add return false\n");
...
static bool dn_pas_media_category_poll (uint_t port, cps_api_object_t obj)
{
    ...
    pcategory = dn_pas_category_get(identifier);
...
static bool dn_pas_media_type_poll (uint_t port, cps_api_object_t obj)
{
    ...
    EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Port(%d) Media type(%u)\n",
            port, type);
...
bool dn_pas_media_high_power_mode_poll (uint_t port, cps_api_object_t obj)
{
    ...
    if (pas_sdi_media_module_control_status_get(mtbl->res_hdl,
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "%s - %d, Invalid port or SDI \
                handle, port(%d) handle (%p)\n", __FUNCTION__, __LINE__,
                port, mtbl->res_hdl);
    ...
    if(mtbl->res_data->high_power_mode != mode) {
        ...
        if (obj != NULL) {
            if (cps_api_object_attr_add(obj,
                ...
                EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "%s - %d, Function \
                        cps_api_object_attr_add return false\n", __FUNCTION__,
                        __LINE__);
        ...
        cps_api_object_t obj, BASE_PAS_MEDIA_t attr_id, bool *ret)
{
    ...
    if ((hdl == NULL) || (memp == NULL)) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid param, hdl(%p) \
                memp(%p)\n", hdl, memp);
    ...
    if (pas_sdi_media_vendor_info_get(hdl, type, memp, size) != STD_ERR_OK) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "sdi_media_vendor_info_get"
                " Failed, hdl(%p), type(%d), memp(%p), size(%d)\n",
                hdl, type, memp, size);
        ...
        sdi_media_module_monitor_t type)
{
    ...
    if (pas_sdi_media_module_monitor_get(mtbl->res_hdl, type, &value)
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid port or SDI handle, \
                port(%d) handle (%p)\n", port, mtbl->res_hdl);
        ...
        sdi_media_channel_monitor_t type)
{
    ...
    if (dn_phy_is_media_channel_valid(port, channel) == false) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid channel, channel %d\n",
                channel);
        return false;
    }
        ...
        uint_t channel, cps_api_object_t obj)
{
    ...
    if (dn_phy_is_media_channel_valid(port, channel) == false) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid channel, \
                port (%d), channel %d\n", port, channel);
        return false;
    }
    ...
    if (pas_sdi_media_tx_control_status_get(mtbl->res_hdl, channel,
                &status) != STD_ERR_OK) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid SDI handle, \
                port(%d) handle (%p) channel (%d)\n", port,
                mtbl->res_hdl, channel);
        return false;
    }
        ...
        cps_api_object_t obj)
{
    ...
    if (dn_pas_media_channel_tx_control_status_poll(port, channel, obj)
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS",
                "dn_pas_media_channel_tx_control_status_poll failed\n");
        ret = false;
    }
    ...
    if (dn_pas_media_channel_monitor_poll(port, channel,
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS",
                "dn_pas_media_channel_monitor_poll failed\n");
    ...
    if (dn_pas_media_channel_monitor_status_poll(port, channel, &mstatus)
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS",
                "dn_pas_media_channel_monitor_status_poll failed\n");
        ret = false;
    }
    ...
    if (dn_pas_media_channel_monitor_status_check(port, channel, obj,
                mstatus) == false) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS",
                "dn_pas_media_channel_monitor_status_check failed\n");
        ret = false;
    }
    ...
    if (dn_pas_media_channel_status_poll(port, channel, &status)
            == false) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS",
                "dn_pas_media_channel_status_poll failed\n");
        ret = false;
    }
    ...
    if (dn_pas_media_channel_status_check(port, channel, obj,
                status) == false) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS",
                "dn_pas_media_channel_status_check failed\n");
        ret = false;
    }
...
static bool dn_pas_media_rtd_poll(uint_t port, cps_api_object_t obj)
{
    ...
    if (dn_pas_media_module_monitor_poll(port, SDI_MEDIA_TEMP) == false) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", 
                "dn_pas_media_module_monitor_poll failed\n");
    ...
    if (dn_pas_media_module_monitor_status_poll(port, &status) == false) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS",
                "dn_pas_media_module_monitor_status_poll failed\n");
        ret = false;
    }
    ...
    if (dn_pas_media_monitor_status_check(port, obj, status) == false) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS",
                "dn_pas_media_monitor_status_check failed\n");
        ret = false;
    }
        ...
        BASE_PAS_MEDIA_t attr_id, bool *ret) 
{
    ...
    if ((hdl == NULL) || (memp == NULL)){
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid handle(%p) or \
                memp(%p)\n", hdl, memp);
    ...
    if(pas_sdi_media_parameter_get(hdl, type, &value) != STD_ERR_OK) { 
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "sdi_media_parameter_get"
                "failed, hdl(%p), type(%d)\n", hdl, type);
        ...
        *memp = value;
        ...
        if (obj != NULL) {
            ...
            if (cps_api_object_attr_add_u32(obj, attr_id, value) == false) {
                ...
                EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Function \
                        cps_api_object_attr_add_u32 return false\n");
        ...
        uint_t channel, uint_t *status)
{
    ...
    *status = 0;
    ...
    if (pas_sdi_media_channel_status_get(mtbl->res_hdl, channel,
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "sdi_media_channel_status_get \
                failed, handle (%p), port(%d), channel(%d)\n", 
                mtbl->res_hdl, port, channel);
        return false;
    }
        ...
        uint_t *status)
{
    ...
    if (status == NULL) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid param, port(%d)\n", port);
        return false;
    }
    ...
    *status = 0;
    ...
    if (pas_sdi_media_module_monitor_status_get(mtbl->res_hdl, flags, status)
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "sdi_media_module_monitor_status_get \
                failed, handle (%p), port(%d)\n", mtbl->res_hdl, port);
...
static bool dn_pas_media_product_info_poll (uint_t port, cps_api_object_t obj)
{
    ...
    if (pas_sdi_media_dell_product_info_get(mtbl->res_hdl, &prod_info) 
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "sdi_media_dell_product_info_get \
                failed, port(%d)\n", port);
...
static bool dn_pas_media_oir_poll (uint_t port, cps_api_object_t obj) 
{
    ...
    if (dn_pas_media_presence_poll(port, obj) == false) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "dn_pas_media_presence_poll \
                failed, port(%d)\n");
    ...
    dn_pas_media_high_power_mode_set(port, true);

    if (dn_pas_media_category_poll(port, obj) == false) {

        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Category poll failed \
                port(%d)\n", port);
    ...
    if (dn_pas_media_wavelength_poll(port, obj) == false) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Wave length poll failed \
                port(%d)\n", port);
    ...
    if (dn_pas_media_type_poll(port, obj) == false) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Media type poll failed \
                port(%d)\n", port);
    ...
    if (dn_pas_media_capability_poll(port, obj) == false) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Capability poll failed \
                port(%d)\n", port);
    ...
    if (dn_pas_media_vendor_name_poll(port, obj) == false) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Vendor Name poll failed \
                port(%d)\n", port);
    ...
    if (dn_pas_media_vendor_oui_poll(port, obj) == false) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Vendor OUI poll failed \
                port(%d)\n", port);
...
void dn_pas_phy_media_channel_poll (uint_t port, uint_t channel, bool publish)
{
    ...
    if (dn_pas_media_channel_rtd_poll(port, channel, obj) == false) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "dn_pas_media_channel_rtd_poll failed\n");
    }
...
void dn_pas_phy_media_poll (uint_t port, bool publish) 
{
    ...
    if (dn_pas_media_oir_poll(port, obj) == false) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "OIR poll failed, \
                port(%d)\n", port);
    ...
    if (dn_pas_phy_media_is_present(port) == true) {
        ...
        if (rtd_poll == true) {
            if (dn_pas_media_rtd_poll(port, obj) == false) {
                EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Media real time data poll \
                        failed, port(%d)\n", port);
            }

            if (dn_pas_media_high_power_mode_poll(port, obj) == false) {
                EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Media High power mode poll \
                        failed, port(%d)\n", port);
            }
        }
    }
    ...
    if ((dn_pas_phy_media_is_present(port) == true) 
        ...
        if (dn_pas_media_data_poll(port, obj) == false) {
            ...
            EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Media data poll failed \
                    port(%d) false\n", port);
        ...
        if (dn_pas_media_threshold_poll(port, obj) == false) {
            ...
            EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Media threshold \
                    poll failed port(%d) false\n", port);
        }
        ...
        cps_api_object_t obj, void * res_data)
{
    ...
    if ((obj == NULL) || (res_data == NULL)
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid param, memp(%p) obj(%p) \
                res_data (%p)\n", memp, obj, res_data);
    ...
    if (list == NULL) { 
        for (cnt = PAS_MEDIA_MEMBER_START; cnt < max_count; cnt++) {
            if (dn_pas_media_obj_attr_add(&memp[cnt], obj, res_data)
                ...
                EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "cps_api_object_attr_add \
                        failed, attr(%d), cnt(%d).\n", 
                        memp[cnt].ident, cnt);
        ...
        for (cnt = 0; cnt < count; cnt++) {
            ...
            if (inx >= max_count) { 
                ...
                EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid attr_id (%d)\n",
                        list[cnt]);
        ...
        cps_api_object_t req_obj)
{
    ...
    if (((mtbl = dn_phy_media_entry_get(port)) == NULL) 
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid port(%d) ch(%d)\n", 
                port, channel);
    ...
    if ((obj = cps_api_object_create()) == NULL) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "CPS object creation failed, \
                port(%d) ch(%d)\n", port, channel);
        return false;
    }
    ...
    if (dn_pas_is_media_obj_empty(req_obj, BASE_PAS_MEDIA_CHANNEL_OBJ)
        ...
        if (dn_pas_media_obj_all_attr_add(media_ch_mem_info,
            ...
            EV_LOG_INFO(ev_log_t_PAS, 3, "PAS",
                    "dn_pas_media_channel_obj_all_attr_add failed\n");
    ...
    if (!cps_api_object_list_append(param->list, obj)) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "List append failed port(%d) \
                ch(%d)\n", port, channel);
        ...
        bool state)
{
    ...
    if (pas_sdi_media_tx_control(mtbl->res_hdl, channel, state) 
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "sdi_media_tx_control \
                failed. Port(%d), channel(%d), state(%d) \n",
                port, channel, state);
        ...
        BASE_IF_SPEED_t speed)
{
    ...
    if ((sdi_speed = dn_pas_to_sdi_capability_conv(speed))
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Invalid speed(%d) - port(%d),\
                channel(%d)\n", speed, port, channel);
    ...
    if (pas_sdi_media_led_set(mtbl->res_hdl, channel, sdi_speed) != STD_ERR_OK) {
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "sdi_media_led_set failed,\
                Port(%d), channel(%d), sdi_speed(%d) \n", port, channel, sdi_speed);
...
bool dn_pas_media_phy_autoneg_set (uint_t port, uint_t channel)
{
    ...
    if (sdi_media_phy_autoneg_set(mtbl->res_hdl, channel, type, 
                                  autoneg) != STD_ERR_OK) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Setting autoneg (%d) \
                failed in SDI\n", port);
        return false;
    }
...
bool dn_pas_media_phy_supported_speed_set (uint_t port, uint_t channel)
{
    ...
    if (sdi_media_phy_speed_set(mtbl->res_hdl, channel, type, sdi_sup_speed,
                count) != STD_ERR_OK) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Setting supported speed (%d) \
                failed in SDI\n", port);
        return false;
    }
...
bool dn_pas_media_phy_interface_mode_set (uint_t port, uint_t channel)
{
    ...
    if (sdi_media_phy_mode_set(mtbl->res_hdl, channel, type, mode) != STD_ERR_OK) {
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "Setting interface mode (%d) \
                failed in SDI\n", port);
        return false;
    }
...
bool dn_pas_media_high_power_mode_set (uint_t port, bool mode)
{
    ...
    if (pas_sdi_media_module_control(mtbl->res_hdl, SDI_MEDIA_LP_MODE,
        ...
        EV_LOG_INFO(ev_log_t_PAS, 3, "PAS", "sdi_media_module_control \
                failed. Port(%d), mode(%d), lr_mode(%d)\n",
                port, mode, lr_mode);
...
bool dn_pas_media_lockdown_handle (bool lockdown)
{
    ...
    for (index = PAS_MEDIA_START_PORT; index <= max_count; index++) {
        ...
        if (((mtbl = dn_phy_media_entry_get(index)) == NULL)
                || (mtbl->res_data->present == false)){
            continue;
        }
        ...
        if ((mtbl->res_data->dell_qualified == false)
            ...
            if (dn_pas_media_transceiver_state_set(index, lockdown) == false) {
                ret = false;
            }
        }
    }
...
void dn_pas_phy_media_res_free(void)
{
    ...
    for (port = PAS_MEDIA_START_PORT; port <= dn_phy_media_count_get(); port++) {
        ...
        if (phy_media_tbl[port].channel_data != NULL) {
            for (cnt = PAS_MEDIA_CH_START; cnt < phy_media_tbl[port].channel_cnt;
                ...
                cps_api_key_init(&key, cps_api_qualifier_OBSERVED,
                        cps_api_obj_CAT_BASE_PAS, BASE_PAS_MEDIA_CHANNEL_OBJ,
                        PAS_MEDIA_CHANNEL_INST_KEY_LEN, slot, port, cnt);
        ...
        cps_api_key_init(&key, cps_api_qualifier_OBSERVED,
                cps_api_obj_CAT_BASE_PAS, BASE_PAS_MEDIA_OBJ,
                PAS_MEDIA_INST_KEY_LEN, slot, port);
        dn_pas_res_remove(&key);
