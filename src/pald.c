...
static t_pald_thread_list thread_main_functions [] = {
    ...
    //pas remote poller thread temp fix as nas is not preset
...
static char PAS_CONFIG_FILENAME_DFLT[] = "/etc/sonic/pas/config.xml";
...
{ 
    dn_pas_config_init(config_filename, cps_hdl);
    ...
    for (; thread_idx < total_thread_num; thread_idx++) {
      if (thread_main_functions[thread_idx].main_thread != NULL) {
          ...
          if (std_thread_create(&pas_thread_entry[thread_idx])!=STD_ERR_OK) {
              EV_LOG_ERR(ev_log_t_PALD,
                         3,
                         "PALD",
                         "%s,%d Error creating thread\n",
                         __FUNCTION__,
                         __LINE__
                         );
...
static t_std_error dn_pald_init(int argc, char *argv[])
{
    ...
    {
        EV_LOG_ERR(ev_log_t_PALD, 1, "PALD", 
                   "%s sdi_sys_init failed at try number: %d\n",
                   __FUNCTION__,
                   (sdi_init_try_count+1)
                   );
    ...
    if(sdi_init_try_count >= sdi_init_max_try) {
        ...
        EV_LOG_ERR(ev_log_t_PALD, 1, "PALD", 
               "%s sdi_sys_init failure occured, giving up\n",
                __FUNCTION__
                );
        return STD_ERR(PAS, FAIL, 0);
    }
    ...
    EV_LOG_ERR(ev_log_t_PALD, 1, "PALD", 
               "%s sdi_sys_init succeeded at try number: %d\n",
                __FUNCTION__,
                (sdi_init_try_count+1)
               );
    
    if (cps_api_operation_subsystem_init(&cps_hdl, 1)!= cps_api_ret_code_OK) {
        EV_LOG_ERR(ev_log_t_PAS, 1, "PAS", 
                   "%s, cps_api_operation_subsystem_init failed\n",
                  __FUNCTION__); 
        return STD_ERR(PAS, FAIL, 0);
    }
    ...
    do {
       ...
       if (STD_IS_ERR(ret)) {
          EV_LOG_ERR(ev_log_t_PALD, 1, "PALD", 
                     "%s pas_config_file_handle() failed\n",
                     __FUNCTION__);
          break;
       }
       ...
       if (STD_IS_ERR(ret)) {
            EV_LOG_ERR(ev_log_t_PALD, 1, "PALD", "%s, pald thread init failed\n", 
                      __FUNCTION__);
