...
static bool dn_pas_chassis_notify(pas_chassis_t *rec)
{
    ...
    cps_api_key_init(cps_api_object_key(obj),
...
{
    cps_api_key_t    *key = &param->keys[key_idx], rec_key[1];
    pas_chassis_t    *rec;
    cps_api_object_t obj;
                    ...
                    uint8_t                       reboot_type)
{
    ...
    rec = (pas_chassis_t *) dn_pas_res_get(rec_key);
    if (rec == 0) {
        /* Not found */
                               ...
                               cps_api_object_t              obj)
{
    ...
    if (reboot_type != PLATFORM_REBOOT_TYPE_COLD &&
        ...
        EV_LOG_ERR(ev_log_t_PAS, 3, "PAS", 
                   "%s, invalid reboot type %u\n", __FUNCTION__, reboot_type);
